...
from mimdepth.datasets.transforms import get_transforms
from mimdepth.utils.depth import inv2depth, post_process_inv_depth, compute_depth_metrics
from mimdepth.utils.horovod import print0, world_size, rank, on_rank_0
from mimdepth.utils.image import flip_lr
from mimdepth.utils.load import load_class, load_class_args_create, \
    load_network, filter_args
from mimdepth.utils.logging import pcolor
from mimdepth.utils.reduce import all_reduce_metrics, reduce_dict, \
    create_dict, average_loss_and_metrics
from mimdepth.utils.save import save_depth
from mimdepth.models.model_utils import stack_batch
...
class ModelWrapper(torch.nn.Module):
    ...
    def prepare_model(self, resume=None):
        ...
        self.config.model.depth_net.mim_strategy = self.config.datasets.augmentation.mim_strategy
        self.config.model.pose_net.mim_strategy = self.config.datasets.augmentation.mim_strategy
...
def setup_model(config, prepared, image_shape=(192, 640), **kwargs):
    ...
    model = load_class(config.name, paths=['mimdepth.models',])(
        **{**config.loss, **kwargs})
...
def setup_dataset(config, mode, requirements, **kwargs):
    ...
    for i in range(len(config.split)):
        # path_split : train, validation, test data file list file's path
        ...
        if config.dataset[i] == 'KITTI':
            from mimdepth.datasets.kitti_dataset import KITTIDataset
            dataset = KITTIDataset(
                config.path[i], path_split,
                **dataset_args, **dataset_args_i,
            )
        # Realsense Dataset (Additional)
        elif config.dataset[i] == 'RS':
            from mimdepth.datasets.RS_dataset import RSDataset
            dataset = RSDataset(
                config.path[i], path_split,
                **dataset_args, **dataset_args_i,
            )
