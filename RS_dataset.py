...
from mimdepth.datasets.kitti_dataset_utils import \
    pose_from_oxts_packet, read_calib_file, transform_from_rot_trans
from mimdepth.utils.image import load_image
from mimdepth.geometry.pose_utils import invert_pose_numpy
...
class RSDataset(Dataset):
    ...
    def __init__(self, root_dir, file_list, train=True,
        ...
        # data : dataset file path list. detailed .png path
        with open(file_list, "r") as f:
            data = f.readlines()

        self.paths = []
        # Get file list from data
        # fname : just one file path.
        for i, fname in enumerate(data):
            ...
            if not self.with_depth:
                ...
                ######## _get_depth_file is very important. Make directories with following function.
...
########################################################################################################################
    ...
    def _get_depth_file(self, image_file):
        ...
        # 숫자 부분이 없는 경우의 처리
            number_part = ''
        depth_file = image_file.replace(f'data/{number_part}.png', 'proj_depth/groundtruth/gt.png')
